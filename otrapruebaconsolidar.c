...
void read_input_file(char* filename, input_row bet[], int *num_rows) {
    ...
    // Leemos la primera línea del archivo que contiene los nombres de los campos
    fgets(line, sizeof(line), file);
    int j=*num_rows;
    // Iteramos por cada línea del archivo
    while (fgets(line, sizeof(line), file))
    {
        // Utilizamos strtok para separar los campos de la línea
        char* field = strtok(line, ";");
        int i = 0;

        // Iteramos por cada campo de la línea y lo almacenamos en la estructura Bet
        while (field != NULL)
        {
            switch (i)
            {
            ...
            // Pasamos al siguiente campo de la línea
            field = strtok(NULL, ";");
            i++;
        }

        // Aquí puedes utilizar la estructura Bet para lo que necesites
...
void consolidate_data(input_row input_rows[], int *num_input_rows,
                      output_row output_rows[], int *num_output_rows, char *filename) {
    int i, j;
    output_row row;
    int found=0;
    *num_output_rows = 0;
    ...
    if (anio3 < anio4) {
        ...
        strcpy(row.fecha_fin, input_rows[i].fecha_fin);
         ...
         strcpy(row.fecha_fin, output_rows[j].fecha_fin);
    } else if (minuto3 > minuto4) {
        strcpy(row.fecha_fin, input_rows[i].fecha_fin);
    } else if (segundo3 < segundo4) {
         strcpy(row.fecha_fin, output_rows[j].fecha_fin);
    } else if (segundo3 > segundo4) {
        strcpy(row.fecha_fin, input_rows[i].fecha_fin);
    } else {
...
void write_output_file(char* filename, output_row rows[], int *num_rows) {
...
int main() {
    char input_filenames[3][MAX_FILENAME_LEN] = {
        "ficheros/CA001_BET365_POKER_ON_30032023_1.csv"
        };
    ...
    for (int i = 0; i < num_input_files; i++) {
        read_input_file(input_filenames[i], bet, &num_input_rows);
    }
    printf("\n\n");
    consolidate_data(bet, &num_input_rows, output_rows, &num_output_rows, input_filenames[0]);
    write_output_file(output_filename, output_rows, &num_output_rows);
