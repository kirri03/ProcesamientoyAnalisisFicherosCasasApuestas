...
void* patron2(void* arg)
{
    ...
    // Almacenar los valores de IdUsuario y IdJuego en un array
    char usuarios[MAX_USERS][30];
    char juegos[MAX_USERS][30];
    int num_usuarios = 0;

    while (fgets(line, MAX_LINE_LENGTH, file)) {
        // Extraer los valores de cada campo
        token = strtok(line, ";");
        strcpy(juegos[num_usuarios], token);

        token = strtok(NULL, ";");
        token = strtok(NULL, ";");

        token = strtok(NULL, ";");
        strcpy(usuarios[num_usuarios], token);

        token = strtok(NULL, ";");
        strcpy(juegos[num_usuarios], token);

        // Comprobar si el IdUsuario tiene más de un IdJuego distinto
        int encontrado = 3;

        for (int i = 0; i < num_usuarios; i++) {
            if (strcmp(usuarios[num_usuarios], usuarios[i]) == 0 && strcmp(juegos[num_usuarios], juegos[i]) != 0) {
                encontrado = 1;
                break;
            }
        }
