...
void funcion_consolidar(char* filename){
            ...
            while (fgets(line, sizeof(line), file))
            {
                ...
                printf("Casa Apuesta: %s, Bet ID: %s, Fecha inicio: %s, Fecha fin: %s, Usuario ID: %s, Sesion Juego ID: %s, Participacion: %d, Apuesta: %.2f, Estado: %.2f\n", input_rows[num_input_rows].casa_apuesta,input_rows[num_input_rows].id_apuesta, input_rows[num_input_rows].fecha_inicio,input_rows[num_input_rows].fecha_fin, input_rows[num_input_rows].id_usuario,input_rows[num_input_rows].id_sesion_juego, input_rows[num_input_rows].participacion, input_rows[num_input_rows].apuesta, input_rows[num_input_rows].estado);
                num_input_rows++;
            }
            fclose(file);
