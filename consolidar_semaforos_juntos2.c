...
void* consolidar(void* arg)
{
    ...
    while ((ent = readdir(dir)) != NULL)   // lee cada archivo en el directorio
    {
        ...
        printf("La funcion saca el num %d\ns",number);
        if (*endptr == '_' && ent->d_type == DT_REG && num_casa==number)   // verifica si es un archivo regular
        {
            ...
            strcpy(filename,"consolidated.csv");
            ...
            fclose(file);
            strcpy(destino,INVENTORY_FILE);
            strcat(destino,ent->d_name);
            rename(filename,INVENTORY_FILE);
