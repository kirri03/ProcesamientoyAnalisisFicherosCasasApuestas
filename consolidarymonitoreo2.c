...
void leerfichero() {
    // Abre el archivo de configuraciï¿½n
    FILE* conf_file = fopen("config.conf", "r");
    if (conf_file == NULL) {
        printf("No se pudo abrir el archivo de configuraciï¿½n\n");
        exit(1);
    }

    // Lee y procesa cada lï¿½nea de configuraciï¿½n
    char buffer[MAX_LINE_LEN];
    char* name;
    char* value;
    while (fgets(buffer, MAX_LINE_LEN, conf_file) != NULL) {
        // Ignora las lï¿½neas en blanco o que comiencen con #
        if (buffer[0] == '\n' || buffer[0] == '#') {
            continue;
        }
        ...
        *(end + 1) = 0;
        ...
        if (strcmp(name, "PATH_FILES") == 0) {
            // Configura la variable PATH_FILES con el valor leï¿½do
            strcpy(PATH_FILES,value);
        } else if (strcmp(name, "INVENTORY_FILE") == 0) {
            // Configura la variable INVENTORY_FILE con el valor leï¿½do
		INVENTORY_FILE=strdup(value); 
        } else if (strcmp(name, "LOG_FILE") == 0) {
            // Configura la variable LOG_FILE con el valor leï¿½do
            LOG_FILE = strdup(value);
        } else if (strcmp(name, "NUM_PROCESOS") == 0) {
            // Configura la variable NUM_PROCESOS con el valor leï¿½do
            NUM_PROCESOS= atoi(value);

        } else if (strcmp(name, "SIMULATE_SLEEP") == 0) {
            // Configura la variable SIMULATE_SLEEP con el valor leï¿½do
            SIMULATE_SLEEP = atoi(value);

        }
    }

    // Cierra el archivo de configuraciï¿½n
    fclose(conf_file);
    ...
    while ( 1 ) {
		...
		strcat(input_filename,event->name);
		 ...
		 read_input_file(input_filename, bet, &num_input_rows);
    		...
    		write_output_file("consolidated2.csv", output_rows, &num_output_rows, &ant_output_rows);
